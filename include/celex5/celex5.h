...
public:
	...
	enum CeleX5Mode {
        ...
        Event_In_Pixel_Timestamp_Mode = 1,   //modified by Chen Guanzhou
	...
	*/
	...
	/*
	* Set and get sensor mode (Loop mode)  by Chen Guanzhou
	*/
	void setSensorLoopMode(CeleX5Mode mode, int loopNum, int device_index); //LopNum = 1/2/3
	CeleX5Mode getSensorLoopMode(int loopNum, int device_index); //LopNum = 1/2/3
	void setLoopModeEnabled(bool enable, int device_index);
	bool isLoopModeEnabled(int device_index);
...
private:
    ...
    //by Chen Guanzhou
    bool                           m_bLoopModeEnabled[MAX_SENSOR_NUM];
};
